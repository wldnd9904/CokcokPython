    ...
    for file_name in os.listdir(data_dir):
        if file_name.endswith('.json'):
            ...
            with open(file_path, 'r') as json_file:
                ...
                combined_data = np.column_stack((x_freq_positive,x_amp_positive,y_freq_positive,y_amp_positive,z_freq_positive,z_amp_positive,pitch_freq_positive,pitch_amp_positive,roll_freq_positive,roll_amp_positive))

                # 데이터 정규화
                combined_data = StandardScaler().fit_transform(combined_data)

                if file_name.startswith('A'):
                    label = 1  # 파일 이름이 'A'로 시작하면 1로 설정
                else:
...
# 모델 학습
model.fit(train_data, train_labels, epochs=500, batch_size=32, validation_split=0.2)
prob_dict = {}
...
for key, average in rank:
    if key in actual_rank:
        print(f"{cnt}: {key}: {average:.6f} ({actual_rank[key]})")
        rank_distance += abs(cnt - actual_rank[key])
        cnt += 1
